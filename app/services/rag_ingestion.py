...
class GeminiEmbeddingProvider:
    ...
    async def embed(self, text_value: str, *, retry: bool = False) -> list[float]:
        ...
        body = {
            ...
            "outputDimensionality": settings.RAG_EMBEDDING_DIM,
        }
        attempts = max(1, 1 + (settings.RAG_EMBEDDING_RETRIES if retry else 0))
        ...
        for attempt in range(attempts):
            try:
                async with httpx.AsyncClient(timeout=settings.RAG_EMBEDDING_TIMEOUT_SECONDS) as client:
                    ...
                    if len(values) != settings.RAG_EMBEDDING_DIM:
                        raise RuntimeError(f"Gemini returned {len(values)} embedding dimensions; expected {settings.RAG_EMBEDDING_DIM}")
